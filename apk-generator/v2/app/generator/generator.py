...
class Generator:
    ...
    def generate(self, should_notify=True):
        ...
        if self.build_type == 'googleplay':
            build_release = 'Googleplay'
        else:
            build_release = 'Fdroid'
        self.run_command([os.path.abspath(self.config['BASE_DIR'] + '/scripts/build_apk.sh'), build_tools_path, self.build_type, build_release])
